...
--Fixed join issue with PS_PYMNT_ADVICE and PS_PYMNT_VCHR_XREF
--USE THIS VERSION
--Added Voucher and XREF Information
SELECT PD.PMT_ID, PD.PMT_SOURCE, PD.BUSINESS_UNIT, PD.SRC_REF_ID, PD.AMOUNT, PD.AMOUNT_CURRENCY_CD, PD.PYMNT_DT, PD.VALUE_DT, PD.ESTIMATED_DUE_DATE, 
       ...
       (SELECT RTRIM(XMLAGG(XMLELEMENT(E, PACK.PMT_ACK_FILE_ID || ' - '|| PACK.HEADER_ID || ' - '|| PACK.FILE_ACK_STATUS || ' - ' || X14.XLATLONGNAME || ' - '|| 
        ...
        (SELECT RTRIM(XMLAGG(XMLELEMENT(E, VC.BUSINESS_UNIT  || ' - '|| VC.VOUCHER_ID || ' - '|| VL.VOUCHER_LINE_NUM || ' - '|| DL.DISTRIB_LINE_NUM || ' - '|| 
       ...
       (SELECT RTRIM(XMLAGG(XMLELEMENT(E, VX.BUSINESS_UNIT || ' - ' || VX.VOUCHER_ID || ' - ' || VX.PYMNT_CNT || ' - ' || VX.PYMNT_ID || ' - ' || VX.PAID_AMT_GROSS
