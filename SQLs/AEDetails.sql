...
--Version 2
SELECT CASE WHEN AE_STMT_TYPE = 'C' THEN
       CASE
          WHEN SD1.AE_DO_APPL_ID <> ' ' THEN '#Jump To ->' || SD1.AE_DO_APPL_ID || '->' || SD1.AE_DO_SECTION 
          WHEN SD1.AE_DO_APPL_ID = ' ' AND SD1.AE_DYNAMIC_DO = 'Y' THEN '##Dynamic Call -> Check preceding PCode Action'          
       END
       ELSE SEC.AE_APPLID || '->' || SEC.AE_SECTION || '->' || SD1.AE_SEQ_NUM || '->' || SD1.AE_STEP || '->' || AE_STMT_TYPE
       END "FLOW#",
       SEC.AE_APPLID, SEC.AE_SECTION, SECDTL.MARKET, SECDTL.DBTYPE, SECDTL.EFFDT "Section EFFDT", SECDTL.EFF_STATUS, SECDTL.DESCR "Section DESCR",
       SD1.AE_SEQ_NUM "AE Step SEQ", SD1.AE_STEP, SD1.DESCR "Step DESCR",  
       CASE
          WHEN SEC.AE_SECTION = 'MAIN' THEN 1
--          WHEN SEC.AE_SECTION <> 'MAIN' THEN 1
          WHEN SEC.AE_SECTION <> 'MAIN' THEN (1 + ROW_NUMBER () OVER (PARTITION BY SEC.AE_APPLID, SEC.AE_SECTION, SD1.AE_STEP ORDER BY SEC.AE_APPLID, SEC.AE_SECTION, SD1.AE_STEP))
       END "SECT#",       
       CASE 
          WHEN SD1.AE_COMMIT_AFTER = 'D' THEN 'Default Action' WHEN SD1.AE_COMMIT_AFTER = 'N' THEN 'Later' WHEN SD1.AE_COMMIT_AFTER = 'Y' THEN 'After this Step'
       END "Commit After", SD1.AE_COMMIT_FREQ "Commit Freq",
       CASE 
          WHEN SD1.AE_ABEND_ACTION = 'A' THEN 'Abort' WHEN SD1.AE_ABEND_ACTION = 'B' THEN 'Break' 
          WHEN SD1.AE_ABEND_ACTION = 'I' THEN 'Ignore Error' WHEN SD1.AE_ABEND_ACTION = 'S' THEN 'Suppress Error'
       END "On Error", SD1.AE_ACTIVE_STATUS, SD1.EFFDT "Step EFFDT", SD1.AE_DYNAMIC_DO "Dynamic DO", SD1.AE_PC_ON_FALSE "On False PC", SD1.AE_ON_NOROWS "On NoRows", SD1.AE_DO_APPL_ID, 
                       SD1.AE_DO_SECTION,
       CASE 
          WHEN AE_STMT_TYPE = 'H' THEN 'DO When'        --DO WHEN 3
          WHEN AE_STMT_TYPE = 'W' THEN 'Do While'       --DO WHILE 9
       ...
       CASE 
          WHEN AE_REUSE_STMT = 'N' THEN 'No' WHEN AE_REUSE_STMT = 'Y' THEN 'Yes' WHEN AE_REUSE_STMT = 'B' THEN 'Bulk Insert'
       END "ReUse Statement", 
       CASE
          WHEN AE_DO_SELECT_TYPE = 'F' AND AE_STMT_TYPE = 'D' THEN 'Select/Fetch' WHEN AE_DO_SELECT_TYPE = 'R' THEN 'Reselect' WHEN AE_DO_SELECT_TYPE = 'Y' THEN 'Restartable'
       END "Do Select Type", STMT.DESCR "Action DESCR", STMT.SQLID, SQTXT.SEQNUM, SQTXT.SQLTEXT, --PCM.PCTEXT,
       SD1.MESSAGE_SET_NBR, SD1.MESSAGE_NBR, SD1.AE_MESSAGE_SET_FLD "MsgSet Field", SD1.AE_MESSAGE_NBR_FLD "MsgNbr Field", 
       SEC.AE_SECTION_TYPE, SEC.AE_PUBLIC_SW, SECDTL.AE_AUTO_COMMIT, SECDTL.PTAESECTIONTRACE, SEC.VERSION, SEC.LASTUPDOPRID, SEC.LASTUPDDTTM, SECDTL.DESCRLONG
FROM PSAESECTDEFN SEC, PSAESECTDTLDEFN SECDTL, PSAESTEPDEFN SD1,
--     (PSAESTEPDEFN SD1 LEFT OUTER JOIN PSPCMTXT PCM ON SD1.AE_APPLID = PCM.OBJECTVALUE1 AND SD1.AE_SECTION = PCM.OBJECTVALUE2 
--                                                                                             AND SD1.MARKET = PCM.OBJECTVALUE3 AND SD1.AE_STEP = PCM.OBJECTVALUE6), 
     (PSAESTMTDEFN STMT LEFT OUTER JOIN PSSQLTEXTDEFN SQTXT ON SQTXT.SQLID = STMT.SQLID AND SQTXT.MARKET = STMT.MARKET AND SQTXT.DBTYPE = STMT.DBTYPE)
WHERE SEC.AE_APPLID = SECDTL.AE_APPLID
  AND SEC.AE_SECTION = SECDTL.AE_SECTION
  AND SEC.AE_APPLID = SD1.AE_APPLID
  AND SEC.AE_SECTION = SD1.AE_SECTION
  ...
  AND SEC.AE_APPLID = 'PC_POADJUST'                                                                                             
ORDER BY SEC.AE_APPLID, SEC.AE_SECTION, SD1.AE_SEQ_NUM, 
         (CASE AE_STMT_TYPE WHEN 'H' THEN 1 WHEN 'W' THEN 2 WHEN 'D' THEN 3 WHEN 'P' THEN 4 WHEN 'S' THEN 5 WHEN 'C' THEN 5 WHEN 'M' THEN 6 WHEN 'X' THEN 8 WHEN 'N' THEN 7 ELSE 9 END);
...
--          WHEN SEC.AE_SECTION <> 'MAIN' THEN 1
--          WHEN SEC.AE_SECTION <> 'MAIN' THEN (1 + ROW_NUMBER() OVER (PARTITION BY SEC.AE_APPLID, SEC.AE_SECTION, SD1.AE_STEP ORDER BY SEC.AE_APPLID, SEC.AE_SECTION, SD1.AE_STEP))
          WHEN SEC.AE_SECTION <> 'MAIN' THEN (1 + ROW_NUMBER() OVER (PARTITION BY SEC.AE_APPLID, SEC.AE_SECTION, SD1.AE_STEP, AE_STMT_TYPE 
                                                                       ORDER BY SEC.AE_APPLID, SEC.AE_SECTION, SD1.AE_STEP, AE_STMT_TYPE))
       ELSE -1
       END "SECT#",
       SD1.AE_SEQ_NUM "AE Step SEQ", SD1.AE_STEP, SD1.DESCR "Step DESCR",
       CASE 
       ...
       CASE
          WHEN AE_DO_SELECT_TYPE = 'F' AND AE_STMT_TYPE = 'D' THEN 'Select/Fetch' WHEN AE_DO_SELECT_TYPE = 'R' THEN 'Reselect' WHEN AE_DO_SELECT_TYPE = 'Y' THEN 'Restartable'
       END "Do Select Type", STMT.DESCR "Action DESCR", STMT.SQLID, SQTXT.SEQNUM, SQTXT.SQLTEXT, --PCM.PCTEXT,
       SD1.MESSAGE_SET_NBR, SD1.MESSAGE_NBR, SD1.AE_MESSAGE_SET_FLD "MsgSet Field", SD1.AE_MESSAGE_NBR_FLD "MsgNbr Field", SEC.AE_SECTION_TYPE, SEC.AE_PUBLIC_SW, 
       SECDTL.AE_AUTO_COMMIT, SECDTL.PTAESECTIONTRACE, SEC.VERSION, SEC.LASTUPDOPRID, SEC.LASTUPDDTTM, SECDTL.DESCRLONG
FROM PSAESECTDEFN SEC, PSAESECTDTLDEFN SECDTL, PSAESTEPDEFN SD1,
--     (PSAESTEPDEFN SD1 LEFT OUTER JOIN PSPCMTXT PCM ON SD1.AE_APPLID = PCM.OBJECTVALUE1 AND SD1.AE_SECTION = PCM.OBJECTVALUE2 
--                                                                                             AND SD1.MARKET = PCM.OBJECTVALUE3 AND SD1.AE_STEP = PCM.OBJECTVALUE6), 
     (PSAESTMTDEFN STMT LEFT OUTER JOIN PSSQLTEXTDEFN SQTXT ON SQTXT.SQLID = STMT.SQLID AND SQTXT.MARKET = STMT.MARKET AND SQTXT.DBTYPE = STMT.DBTYPE)
WHERE SEC.AE_APPLID = SECDTL.AE_APPLID
  ...
  AND (SQTXT.EFFDT IS NULL OR SQTXT.EFFDT = (SELECT MAX(EFFDT) FROM PSSQLTEXTDEFN SQTXT_ED WHERE SQTXT.SQLID = SQTXT_ED.SQLID AND SQTXT.SQLTYPE = SQTXT_ED.SQLTYPE 
                                                                                             ...
                                                                                             AND SQTXT.SEQNUM = SQTXT_ED.SEQNUM))
  AND SEC.AE_APPLID = 'PC_POADJUST'                                                                                             
ORDER BY SEC.AE_APPLID, SECT#, SEC.AE_SECTION, SD1.AE_SEQ_NUM, 
         (CASE AE_STMT_TYPE WHEN 'H' THEN 1 WHEN 'W' THEN 2 WHEN 'D' THEN 3 WHEN 'P' THEN 4 WHEN 'S' THEN 5 WHEN 'C' THEN 5 WHEN 'M' THEN 6 WHEN 'X' THEN 8 WHEN 'N' THEN 7 ELSE 9 END);
...
--Drop Table
begin
    for c1 in (select table_name from user_tables where table_name = 'PS_PP_AE_FLOW') loop
        execute immediate 'drop table '||c1.table_name||'';
    end loop;
end;
/
--Create Table
CREATE TABLE PS_PP_AE_FLOW
AS
SELECT CASE WHEN AE_STMT_TYPE = 'C' THEN
       CASE
          WHEN SD1.AE_DO_APPL_ID <> ' ' THEN '#Jump To-->' || SD1.AE_DO_APPL_ID || '->' || SD1.AE_DO_SECTION 
