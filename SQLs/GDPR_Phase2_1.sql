...
--Drop Table
begin
    for c1 in (select table_name from user_tables where table_name = 'PS_PH2_FSCM_SDF_TBL') loop
        execute immediate 'drop table '||c1.table_name||'';
    end loop;
end;
/
--Create Table
CREATE TABLE PS_PH2_FSCM_SDF_TBL
AS
...
/
--Verify Table
SELECT COUNT(*) FROM PS_PH2_FSCM_SDF_TBL;
/
--Updates
UPDATE PS_PH2_FSCM_SDF_TBL SET "Category" = '##XX##', "Classification" = '##XX##', "Personally Identifiable" = 'No', "Sensitive" = 'No'
...
/
SELECT A.* FROM PS_PH2_FSCM_SDF_TBL A WHERE "Personally Identifiable" = 'Yes' and rownum <= 10;
/
